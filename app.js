...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const timelineVerticalContainer = document.getElementById('timeline-vertical-container');
    ...
    // 1. Initial Render of Blocks
    managementBlocks.forEach(block => {
        const blockEl = document.createElement('div');
        blockEl.className = 'management-block';
        blockEl.id = block.id;
    ...
    const highlightBlocks = (mappedBlockIds, exactMatchBlockId) => {
        document.querySelectorAll('.management-block').forEach(el => {
    ...
    zeitphasen.forEach(phase => {
        ...
        // Vertical timeline button (stacked)
        const vBtn = document.createElement('button');
        vBtn.className = 'timeline-btn';
        vBtn.textContent = phase.name;
        vBtn.dataset.phase = phase.id;
        vBtn.addEventListener('click', () => handlePhaseClick(phase));
        if (timelineVerticalContainer) {
            timelineVerticalContainer.appendChild(vBtn);
        }
    });
    ...
    // Initially highlight the first phase
    if (zeitphasen.length > 0) {
        handlePhaseClick(zeitphasen[0]);
    }
});
