...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const fundInvestments = {}; // key: fund.name, value: Number
    ...
    const parseEuro = (str) => {
        ...
        const parsed = parseFloat(str.toString().replace(/\./g, '').replace(',', '.'));
    ...
    const updateFinancialOverview = () => {
        ...
        // Calculate totals per layer and overall
        managementBlocks.forEach(block => {
            ...
            if (blockTotal > 0) {
                layerTotals[block.id] = blockTotal; // keyed by block ID for tower updates
            }
            totalDistributed += blockTotal;
        });

        const remaining = totalInvestment - totalDistributed;

        // Update displays
        if (totalDistributedDisplay) totalDistributedDisplay.textContent = formatCurrency(totalDistributed);
        if (totalRemainingDisplay) totalRemainingDisplay.textContent = formatCurrency(remaining);

        if (totalRemainingDisplay) {
            const remainingRow = totalRemainingDisplay.closest('.finance-row');
            if (remainingRow) {
                if (remaining < 0) {
                    remainingRow.classList.add('negative-balance');
                } else {
                    remainingRow.classList.remove('negative-balance');
                }
            }
        }

        // Update breakdown list and tower layers
        if (layerBreakdownList) {
            layerBreakdownList.innerHTML = '';

            // First loop: hide all tower layer badges
            document.querySelectorAll('.layer-assigned-amount').forEach(badge => {
                badge.classList.remove('visible');
                badge.textContent = '';
            });

            const activeLayerIds = Object.keys(layerTotals);
            if (activeLayerIds.length === 0) {
                layerBreakdownList.innerHTML = '<li class="empty-breakdown">Noch keine Beträge zugewiesen</li>';
            } else {
                activeLayerIds.forEach(blockId => {
                    const block = managementBlocks.find(b => b.id === blockId);
                    if (block) {
                        const amount = layerTotals[blockId];

                        // Add to breakdown list
                        const li = document.createElement('li');
                        li.innerHTML = `
                            <span class="breakdown-layer-name">${block.title}</span>
                            <span class="breakdown-amount">${formatCurrency(amount)}</span>
                        `;
                        layerBreakdownList.appendChild(li);

                        // Show on tower layer directly
                        const layerEl = document.getElementById(blockId);
                        if (layerEl) {
                            let badge = layerEl.querySelector('.layer-assigned-amount');
                            if (!badge) {
                                badge = document.createElement('div');
                                badge.className = 'layer-assigned-amount';
    ...
    if (totalInvestmentInput) {
        totalInvestmentInput.addEventListener('blur', (e) => {
            ...
            totalInvestment = val > 0 ? val : 0;
            e.target.value = val > 0 ? formatNumberInput(val) : '';
            updateFinancialOverview();
        });

        totalInvestmentInput.addEventListener('focus', (e) => {
            if (totalInvestment > 0) {
                // Remove formatting except comma for easy editing
                e.target.value = totalInvestment.toString().replace('.', ',');
            } else {
                e.target.value = '';
            }
        });
    ...
    const renderFundList = (fundsList) => {
        ...
        if (fundsList && fundsList.length > 0) {
            fundsList.forEach(fund => {
                ...
                const fundKey = fund.name;
                const currentValue = fundInvestments[fundKey] || 0;
                const displayValue = currentValue > 0 ? formatNumberInput(currentValue) : '';
                ...
                li.innerHTML = `
                    ...
                    <div class="fund-input-wrapper">
                        <div class="currency-input-wrapper">
                            <input type="text" class="fund-investment-input" data-fund-key="${fundKey}" placeholder="0,00" inputmode="decimal" value="${displayValue}">
                            <span class="currency-symbol">€</span>
                        </div>
                    </div>
                `;

                const input = li.querySelector('.fund-investment-input');

                input.addEventListener('blur', (e) => {
                    const val = parseEuro(e.target.value);
                    if (val > 0) {
                        fundInvestments[fundKey] = val;
                        e.target.value = formatNumberInput(val);
                    } else {
                        delete fundInvestments[fundKey];
                        e.target.value = '';
                    }
                    updateFinancialOverview();
                });
                ...
                input.addEventListener('focus', (e) => {
                    const val = fundInvestments[fundKey] || 0;
                    if (val > 0) {
